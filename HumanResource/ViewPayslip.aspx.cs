...
public partial class Employee_ViewPayslip : System.Web.UI.Page
{
    ...
    void getPayTerm()
    {
        ...
        SqlCommand mirai = new SqlCommand();
        mirai.Connection = mio;
        mirai.CommandText = "Select ax.PayTermID, CAST(ax.StartingDate as nvarchar(max)) + ' - ' + CAST(ax.EndingDate as nvarchar(max)) as PeriodCovered From (Select PaytermID, FORMAT(StartingDate, 'MMM-dd-yyyy') as StartingDate, FORMAT(EndingDate, 'MMM-dd-yyyy') as EndingDate From PayTerm) ax Where ax.PayTermID IN(Select PayTermID From PayrollRecords)";
