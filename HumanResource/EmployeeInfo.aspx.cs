...
public partial class HumanResource_EmployeeInfo : System.Web.UI.Page
{
    ...
    protected void rbDsc_CheckedChanged(object sender, EventArgs e)
    {
        if (rbDsc.Checked == true)
        {
            ...
            SqlCommand mirai = new SqlCommand();
            mirai.Connection = mio;
            mirai.CommandText = "Select EmployeeID, FirstName, MiddleName, LastName, convert(date,DateEmployed) as DateEmployed, Position FROM Employee WHERE Status = 'Employed' ORDER BY " + ddlCat.SelectedItem.Text + " DESC";
            SqlDataAdapter da = new SqlDataAdapter(mirai);
            DataSet ds = new DataSet();
            da.Fill(ds, "Employee");
            lvEmployees.DataSource = ds;
        ...
        else if (rbAsc.Checked == true)
        {
            ...
            SqlCommand mirai = new SqlCommand();
            mirai.Connection = mio;
            mirai.CommandText = "Select EmployeeID, FirstName, MiddleName, LastName, convert(date,DateEmployed) as DateEmployed, Position FROM Employee WHERE Status = 'Employed' ORDER BY " + ddlCat.SelectedItem.Text + " ASC";
            SqlDataAdapter da = new SqlDataAdapter(mirai);
            DataSet ds = new DataSet();
            da.Fill(ds, "Employee");
            lvEmployees.DataSource = ds;
