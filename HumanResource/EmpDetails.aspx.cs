    ...
    void GetEmpDetails(int ID)
    {
        ...
        if (aki.HasRows)
        {
            while (aki.Read())
            {
                ...
                ltDateEmp.Text = DateTime.Parse(aki["DateEmployed"].ToString()).ToString("MMM-dd-yyyy}");
                ltDateCrt.Text = DateTime.Parse(aki["DateCreated"].ToString()).ToString("MMM-dd-yyyy");
