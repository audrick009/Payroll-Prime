...
public partial class MasterPages_Admin : System.Web.UI.MasterPage
{
    ...
    void GetLeave()
    {
        ...
        com.Parameters.AddWithValue("@DateToday", DateToday.ToString("yyyy-MMM-dd"));
    ...
    void GetPayslipDate()
    {
        ...
        SqlCommand com = new SqlCommand();
        com.Connection = con;
        com.CommandText = "SELECT pr.PayslipID, FORMAT(pt.StartingDate , 'MMM-dd-yyyy') as StartingDate, FORMAT(pt.EndingDate, 'MMM-dd-yyyy') as EndingDate FROM PayrollRecords pr INNER JOIN PayTerm pt ON pr.PayTermID = pt.PayTermID INNER JOIN Employee e ON pr.EmployeeID = e.EmployeeID where pt.EndingDate > @DateToday AND pt.EndingDate < @DateTom AND pr.EmployeeID = @EmpID";
